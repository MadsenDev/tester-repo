...
function applyPlaygroundBuild(player){
  ...
  player.maxHp=1000;player.hp=1000;player.regen=Math.max(player.regen,80);player.armor=Math.max(player.armor,.75);player.magnet*=1.6;
  ...
  if(b.crit)player.crit=Math.max(player.crit,.4);
  if(b.bullet)player.bulletSpeed*=1.5;
  for(const id of ["missile","arc","nova","mines","beam"])if(b[id])player.weapons[id]=5;
...
export function preparePlayerForMode(player,id){
  ...
  player.maxHp+=35;player.hp=player.maxHp;player.damage*=1.22;player.fireRate*=.82;player.magnet*=1.4;player.weapons.missile=1;player.weapons.arc=1;return player
}
...
export function spawnPressure(id,time){
  ...
  if(id==="playground")return .48;
  const arena=activeBossArena();
  if(arena?.suppressRegulars){const ratio=arena.hpRatio??1;if(ratio>.35)return .05;if(ratio>.12)return .16;return .45}
