...
const panel = document.querySelector("#core"),
  grid = document.querySelector("#coreGrid"),
  shards = document.querySelector("#coreShards");
function render() {
  if (!panel || !grid || !shards) return;
  const core = loadCore();
  shards.textContent = core.shards;
  grid.innerHTML = "";
  for (const u of CORE_UPGRADES) {
    const level = core.levels[u.id] || 0,
      cost = coreCost(level),
      maxed = level >= u.max,
      b = document.createElement("button");
    b.className = "core-upgrade";
    b.disabled = maxed || core.shards < cost;
...
document
  .querySelectorAll('[data-nav="core"],#openCore')
