...
export function moveEnemy(e,dt,{player,enemyBullets,particles,time}){
  ...
  if(e.behavior==="shooter"){
    ...
    if(e.shootCd<=0){enemyBullets.push(spawnEnemyProjectile(e.x,e.y,Math.atan2(dy,dx),330,15,3,4));e.shootCd=2.1;for(let i=0;i<4;i++)particles.push(particle(e.x,e.y,"boss"))}
    ...
    if(e.behavior==="orbiter"&&e.shootCd<=0){enemyBullets.push(spawnEnemyProjectile(e.x,e.y,Math.atan2(dy,dx)+.25,175,10,4,5));enemyBullets.push(spawnEnemyProjectile(e.x,e.y,Math.atan2(dy,dx)-.25,175,10,4,5));e.shootCd=1.55}
