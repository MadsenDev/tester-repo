...
function edgeSpawn(w,h){const side=Math.floor(Math.random()*4),m=70;if(side===0)return{x:Math.random()*w,y:-m};if(side===1)return{x:w+m;y=Math.random()*h}else if(side===2)return{x:Math.random()*w,y:h+m};return{x:-m,y:Math.random()*h}}
...
export function spawnEnemy(w,h,time,eliteBonus=0){
  const pos=edgeSpawn(w,h),summoned=activeSummoner(),available=summoned?ARCHETYPES.filter(a=>["scout","dart","swarm"].includes(a.kind)&&time>=a.unlock):ARCHETYPES.filter(a=>time>=a.unlock),pool=available.length?available:[ARCHETYPES[0]],base=pool[Math.floor(Math.random()*pool.length)],elite=!summoned&&time>150&&Math.random()<Math.min(.55,.04+time/2400+eliteBonus),scale=1+Math.max(0,time-180)*.0014,hp=base.hp*scale*(elite?1.9:1)*(summoned?.78:1);
  return{...pos,px:pos.x,py:pos.y,...base,hp,hpMax:hp,d:base.d*(elite?1.25:1)*(summoned?.82:1),v:Math.round(base.v*(elite?1.8:1)*(summoned?.7:1)),s:base.s*(elite?1.08:1)*(summoned?1.08:1),r:base.r*(elite?1.18:1)*(summoned?.88:1),boss:false,elite,summoned,flash:0,phase:Math.random()*6.28,shootCd:.8+Math.random()*1.4,chargeCd:1.2+Math.random()*1.8}
}
