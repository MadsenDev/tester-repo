...
export function spawnEnemy(w, h, time, eliteBonus = 0, forceElite = false) {
  const pos = edgeSpawn(w, h),
    ...
    available = summoned
      ? ARCHETYPES.filter(
          (a) =>
            ["scout", "dart", "swarm"].includes(a.kind) && time >= a.unlock,
        )
    ...
    trait = elite
      ...
      : null,
    t = trait ? ELITE_TRAITS[trait] : null,
    scale = 1 + Math.max(0, time - 180) * 0.0014,
    hp = base.hp * scale * (t?.hp || 1) * (summoned ? 0.78 : 1);
  const enemy = {
    ...pos,
    px: pos.x,
    py: pos.y,
    ...
    hp,
    hpMax: hp,
    d: base.d * (t?.damage || 1) * (summoned ? 0.82 : 1),
    v: Math.round(base.v * (t?.value || 1) * (summoned ? 0.7 : 1)),
    s: base.s * (t?.speed || 1) * (summoned ? 1.08 : 1),
    r: base.r * (elite ? 1.2 : 1) * (summoned ? 0.88 : 1),
    boss: false,
    elite,
    eliteTrait: trait,
    eliteName: t?.name || "",
    eliteColor: t?.color || base.color,
    summoned,
    flash: 0,
    phase: Math.random() * 6.28,
    shootCd: 0.8 + Math.random() * 1.4,
    chargeCd: 1.2 + Math.random() * 1.8,
  };
