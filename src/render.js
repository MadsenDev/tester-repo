...
function drawEnemy(ctx,e,time){
  ctx.save();ctx.translate(e.x,e.y);ctx.rotate(time*.6+e.phase);ctx.shadowBlur=e.boss?26:e.elite?20:12;ctx.shadowColor=e.color;ctx.fillStyle=e.flash>0?"#fff":e.color;
  const sides=e.kind==="dart"?3:e.kind==="wisp"?4:e.kind==="spitter"?6:e.boss?8:e.r>18?6:5;
  ...
  if(e.boss){ctx.strokeStyle="#ffd3df";ctx.lineWidth=2;polygon(ctx,0,0,e.r+10,8,time);ctx.stroke()}
  ctx.restore();
  if(e.hp<e.hpMax){ctx.fillStyle="rgba(255,255,255,.12)";ctx.fillRect(e.x-e.r,e.y-e.r-10,e.r*2,3);ctx.fillStyle=e.boss?"#ff5f82":"#ff9a72";ctx.fillRect(e.x-e.r,e.y-e.r-10,e.r*2*(e.hp/e.hpMax),3)}
}

export function renderScene(ctx,view,world){
  const {dpr,W,H}=view;
  const {time,shake,state,player,enemies,bullets,enemyBullets,gems,particles,powerups}=world;
  ...
  ctx.strokeStyle="rgba(65,184,220,.07)";ctx.lineWidth=1;
  const grid=48,ox=(-time*8)%grid,oy=(-time*4)%grid;
  ...
  for(const p of particles){const a=Math.max(0,p.life/p.max);ctx.globalAlpha=a;ctx.fillStyle=p.kind==="hurt"?"#ff557c":p.kind==="boss"?"#ffd36f":p.kind==="nova"?"#cf99ff":"#95efff";ctx.fillRect(p.x,p.y,p.size,p.size)}
