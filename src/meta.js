...
export function recordRun({ won, kills, score }) {
  ...
  if (won) stats.wins++;
  stats.kills += kills;
  stats.best = Math.max(stats.best, Math.floor(score));
  localStorage.setItem(STATS_KEY, JSON.stringify(stats));
  const runShards = 1 + Math.min(3, Math.floor(kills / 175));
  awardCore({ bosses: runShards - 1, victory: won });
  return stats;
}
...
export function difficultyConfig(id) {
  if (isDevMode()) return { spawn: 1, damage: 0, score: 1 };
  return id === "chill"
    ? { spawn: 0.92, damage: 0.9, score: 0.9 }
    : id === "intense"
      ? { spawn: 1.38, damage: 1.35, score: 1.45 }
      : { spawn: 1.12, damage: 1.08, score: 1.08 };
}
