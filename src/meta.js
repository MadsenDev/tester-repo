...
export function recordRun({won,kills,score}){const stats=loadStats();if(isDevMode())return stats;stats.runs++;if(won)stats.wins++;stats.kills+=kills;stats.best=Math.max(stats.best,Math.floor(score));localStorage.setItem(STATS_KEY,JSON.stringify(stats));return stats}
export function resetStats(){localStorage.setItem(STATS_KEY,JSON.stringify(DEFAULT_STATS));return{...DEFAULT_STATS}}
export function difficultyConfig(id){if(isDevMode())return{spawn:1,damage:0,score:1};return id==="chill"?{spawn:.84,damage:.82,score:.85}:id==="intense"?{spawn:1.22,damage:1.25,score:1.35}:{spawn:1,damage:1,score:1}}
