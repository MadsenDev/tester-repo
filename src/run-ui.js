...
const startButtons = ["#start", "#restart", "#victoryRestart"]
  .map((s) => document.querySelector(s))
  .filter(Boolean);
let baseline = null,
  suppressPauseKey = false;
...
pause.id = "pause";
pause.className = "panel pause-panel hidden";
...
function showPause(toggleState = true) {
  ...
  document.querySelector("#pauseTime").textContent =
    document.querySelector("#time")?.textContent || "00:00";
  document.querySelector("#pauseSector").textContent =
...
document
  .querySelector("#resumeRun")
  ?.addEventListener("click", () => hidePause(true));
...
document.querySelector("#pauseMainMenu")?.addEventListener("click", () => {
  if (confirm("Abandon this run and return to the command deck?"))
    location.reload();
});
...
function ensureSummary(panel) {
  ...
  unlock.className = "run-unlock hidden";
  unlock.dataset.run = "unlock";
  summary.insertAdjacentElement("afterend", unlock);
  ...
  if (actions && !actions.querySelector(".result-core")) {
    ...
    core.className = "secondary result-core";
    core.textContent = "UPGRADE CORE";
...
function renderResult(panel) {
  ...
  const before = baseline || snapshot(),
    after = snapshot();
  const score = Number(
    panel.querySelector("#finalScore,#victoryScore")?.textContent || 0,
  );
  ...
  const newly = [];
  for (const ship of unlockedShips(after.stats))
    if (!before.ships.has(ship.id)) newly.push(`${ship.name} chassis unlocked`);
  for (const mode of unlockedModes(after.stats))
    if (!before.modes.has(mode.id)) newly.push(`${mode.name} mode unlocked`);
  if (score > before.best) newly.unshift("NEW BEST SCORE");
  const unlock = panel.querySelector('[data-run="unlock"]');
  unlock.textContent = newly.join(" · ");
  unlock.classList.toggle("hidden", newly.length === 0);
}
for (const id of ["gameover", "victory"]) {
  ...
  new MutationObserver(() => {
    if (!panel.classList.contains("hidden"))
      requestAnimationFrame(() => renderResult(panel));
